...
import { deriveSessionKey } from '../pool/sessionKey.js';
import { findPool, reportMemberFailure, reportMemberSuccess } from '../pool/poolResolver.js';
...
compatibleRouter.get('/v1/models', async (req, res) => {
  ...
  // 池模式下 identity 是池名，实际账号在 copilot.identity 上。
  // 失效处理与统计必须用实际账号，否则会打空。
  let effective = { identity, poolId: undefined as string | undefined };
  try {
    const copilot = await copilotAuthManager.getAuth(identity, {
      sessionKey: deriveSessionKey(req, undefined).key,
    });
    accessToken = copilot.accessToken;
    effective = { identity: copilot.identity, poolId: copilot.poolId };
    ...
    await recordRequestStat({ identity: effective.identity, poolId: effective.poolId, path: '/v1/models', success: true });
    ...
    await invalidateUnauthorizedAuth(effective.identity, accessToken, err);
    await recordRequestStat({ identity: effective.identity, poolId: effective.poolId, path: '/v1/models', success: false, failureReason: errorMessage(err) });
...
async function handleForward(req: Request, res: Response, path: CopilotApiPath): Promise<void> {
  ...
  // 会话 key 决定池内落到哪个成员；1:1 模式下这个值不被使用
  const session = deriveSessionKey(req, body);
  const effective = { identity, poolId: undefined as string | undefined };
  try {
    ...
    const upstream = await forwardAuthenticated(
      identity, path, prepared.body, model, diagnostics, prepared.forwardOptions,
      { sessionKey: session.key, onResolved: (r) => Object.assign(effective, r) },
    );
    if (effective.poolId) await reportMemberSuccess(effective.poolId, effective.identity);
    await pipeAndRecord(
      upstream.response, upstream.request, res,
      { identity: effective.identity, poolId: effective.poolId, path, model },
      diagnostics, prepared.pipeOptions,
    );
  } catch (err) {
    if (!(err instanceof HandledUpstreamStreamError)) {
      await recordRequestStat({ identity: effective.identity, poolId: effective.poolId, path, model: requestedModel, success: false, failureReason: errorMessage(err) });
    }
...
async function handleCountTokens(req: Request, res: Response, claudeCodeOptimized: boolean): Promise<void> {
  ...
  const requestedModel = typeof body.model === 'string' ? body.model : undefined;
  // count_tokens 与正式请求共用同一会话 key，确保落到同一成员（否则会白白打散缓存）
  const session = deriveSessionKey(req, body);
  const effective = { identity, poolId: undefined as string | undefined };
  try {
    if (!requestedModel) throw new CopilotModelPathError('Request body must include a string "model".');
    ...
    if (isTokenCountFallbackStatus(upstream.response.status)) {
      ...
      await recordRequestStat({ identity: effective.identity, poolId: effective.poolId, path, model, success: true, inputTokens });
...
async function forwardAuthenticated(
  ...
  poolContext?: { sessionKey: string; onResolved?: (resolved: { identity: string; poolId?: string }) => void },
): Promise<{ response: globalThis.Response; request: PreparedCopilotRequest }> {
  const copilot = await copilotAuthManager.getAuth(identity, {
    sessionKey: poolContext?.sessionKey,
  });
  // 让调用方知道真正用了哪个账号 —— 统计与失效都依赖它
  poolContext?.onResolved?.({ identity: copilot.identity, poolId: copilot.poolId });
  try {
    await assertModelSupportsPath(copilot, path, model, diagnostics);
  } catch (err) {
    await invalidateUnauthorizedAuth(copilot.identity, copilot.accessToken, err);
    if (copilot.poolId) await notePoolFailure(copilot.poolId, copilot.identity, err);
    throw err;
  }
...
export async function pipeAndRecord(
  ...
  upstreamRequest: PreparedCopilotRequest,
  res: Response,
  stat: { identity: string; poolId?: string; path: CopilotApiPath; model?: string },
  diagnostics: ErrorDiagnosticContext,
  options: PipeOptions = {},
): Promise<void> {
...
/**
 * 把上游失败回报给池：429 立刻冷却该成员，其余错误累加失败计数。
 * 成功路径由 pipeAndRecord 侧的 notePoolSuccess 负责。
 */
async function notePoolFailure(poolId: string, identity: string, err: unknown): Promise<void> {
  const pool = await findPool(poolId);
  if (!pool) return;
  const rateLimited = err instanceof CopilotApiError && (err.status === 429 || err.status === 402);
  await reportMemberFailure(pool, identity, { forceCooldown: rateLimited });
}
